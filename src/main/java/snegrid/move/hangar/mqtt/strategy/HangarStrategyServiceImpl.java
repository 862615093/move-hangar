        ...
        switch (commonReceiveCmd.getMsgType()) {
            ...
            case 2100:
                ...
                if (!FlyTaskUtil.sendMessageToMcs(droneNumber, 4104, droneNumber, null == sessionId ? 1L : sessionId, null)) {
                    nettyHandler.sendMessageToAll("通知无人机允许降落指令失败！");
                    logger.error("通知无人机允许降落指令失败！");
                    break;
                }
                //2.流程通知
            ...
            case 2101:
                ...
                if (!FlyTaskUtil.sendMessageToMcs(droneNumber, 4012, droneNumber, null == sessionId ? 1L : sessionId, null)) {
                    nettyHandler.sendMessageToAll("通知MCS开始上传图片指令失败！");
                    logger.error("通知MCS开始上传图片指令失败！");
                    break;
                }
                //2.流程通知
            ...
            case 2006:
                ...
                if (!flyTaskService.lambdaUpdate().eq(FlyTask::getId, lastFlyTask.getId()).set(FlyTask::getTaskEndTime, LocalDateTime.now()).update()) {
                    nettyHandler.sendMessageToAll("机库回收动作完成, 更新任务结束时间失败！");
                    logger.error("机库回收动作完成, 更新任务结束时间失败！");
                    break;
                }
                //2.流程通知
